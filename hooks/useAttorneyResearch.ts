...
export function useAttorneyResearch() {
  ...
  const sendMessage = useCallback(
    async (content: string) => {
      ...
      try {
        ...
        // Build clean history for API (same as useChat)
        ...
        const response = await fetch("/api/chat", {
          ...
          body: JSON.stringify({
            conversationId: null, // ephemeral — not saved to DB
            ...
            for (const line of lines) {
              ...
              try {
                ...
                if (clientKeypairRef.current && enclavePublicKeyRef.current) {
                  const plain = await decryptEnclaveResponse(chunk, clientKeypairRef.current.privateKey, enclavePublicKeyRef.current);
                  accumulated += plain;
                  if (!animating) { animating = true; requestAnimationFrame(animateNext); }
                }
              } catch {}
            }
          }
          displayed = accumulated;
          if (!animating) setMessages((prev) => prev.map((m) => (m.id === aiId ? { ...m, content: accumulated } : m)));
        } else {
          if (!response.body) throw new Error("No body");
          const reader = response.body.getReader();
          const decoder = new TextDecoder();
  ...
  return { messages, isStreaming, sendMessage, stopStreaming, reset, restoreMessages };
}
